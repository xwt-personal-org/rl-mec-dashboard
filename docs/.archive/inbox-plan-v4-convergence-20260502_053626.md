...
- 状态：变更后待执行
- 阻塞项：无
- Last Iteration Summary：模块 11 与模块 12 已完成，当前 dashboard 已支持 backup/archive 详情展示、artifact log、受控源文件删除、前端视觉层级整理与本地验证环境恢复。本轮基于 `paper2` 新增的 `convergence_by_seed` 数据结构，新增 dashboard 收敛曲线展示能力，不重做既有监控主链路、不引入 React/Vite、不展示静态 PNG，直接用 Chart.js 渲染可交互曲线。
...
2. 不直接嵌入 `paper2` 生成的 `figures/convergence_curves.png`；dashboard 使用 benchmark JSON 中的 `convergence_by_seed` 渲染交互式 Chart.js 曲线。
3. 第一版 UI 以 `eval/reward_mean` 为默认且必须实现的收敛曲线。
4. 后端解析层必须兼容 `convergence_by_seed` 的 dict 与 list 两种格式。
5. 每个算法支持多 seed 数据；API 返回 per-seed 与 mean/std 聚合曲线。
6. x 轴使用 `eval_interval * index`，若缺失 `eval_interval`，默认使用 1000。
7. 曲线数据来自当前 selected run 或 selected backup/archive 对应的 benchmark JSON，不新增数据库。
8. `/api/runs` 不扩大 payload；收敛曲线通过独立 endpoint 懒加载。
9. backup/archive 详情页也必须能查看收敛曲线，只要其 archive benchmark JSON 包含 `convergence_by_seed`。
10. 前端算法切换必须复用当前 `currentRunState.results` 中的算法列表；无曲线数据的算法置灰或显示 no data。
11. 保持单文件前端，不拆 React/Vite。
...
- run 数据源规则：
  ...
  4. 若仍无数据，返回：
     ```json
     {
       ...
       "algorithms": [],
       "series": [],
       "missing_reason": "benchmark export has no convergence_by_seed"
     }
     ```
- backup/archive 数据源规则：
  ...
  4. 返回 `source_type = "backup"` 或 `"archive"`。
- metric 参数规则：
...
- UI 行为：
  ...
  - 无数据时显示：`No convergence data found in benchmark JSON. Re-run paper2 benchmark after Patch 7-9.`
- 验证：
  ```bash
...
- 操作：
  1. README 增加“Convergence 曲线”说明：
     ...
     - 需要重跑 `paper2` Patch 7-9 之后的 benchmark 才有数据。
     - 支持 run 与 backup/archive。
     - 支持算法切换。
...
- 手动验收：
  ...
  5. 选择 backup/archive 数据源后，若 archive benchmark JSON 有 `convergence_by_seed`，曲线正常展示。
  6. 旧 benchmark JSON 无 convergence 数据时，页面显示 no data，不报 JS 错误。

---

## 模块 13 验收标准

- [ ] `/api/runs/{run_id}/convergence?metric=reward` 可返回当前 run 的 reward-step 曲线。
- [ ] `/api/backups/{backup_id}/convergence?metric=reward` 可返回 backup/archive benchmark JSON 中的 reward-step 曲线。
- [ ] API 能兼容 `convergence_by_seed` dict 与 list 两种格式。
- [ ] 多 seed 数据能输出 mean/std，并按 `eval_interval` 生成 step。
- [ ] 前端新增 Convergence 面板。
- [ ] 点击算法 tab 后曲线切换，不刷新整个页面。
- [ ] 无 convergence 数据时 UI 友好提示，不影响现有 Reward Comparison / Training Time / Backups / Danger Zone。
- [ ] 删除当前数据源后 convergence chart 被清空或重载，不显示已删除数据。
- [ ] 全量 pytest 通过。
...
1. 本 patch 只对接 `paper2` 已生成的 `benchmark*.json`；不要修改 `paper2` 仓库。
2. 不引入新前端构建系统。
3. 不把 convergence 数据塞入 `/api/runs` 列表，避免 run overview payload 变大。
4. `AlgorithmResult` 可以保持轻量，不强制把 `convergence_by_seed` 放进每个 result；推荐通过 `dashboard/convergence.py` 从 raw benchmark payload 解析。
